...
def add_buttons(name1, name2, name3, name4: str):
    button = [[KeyboardButton(text=f'{name1}'), KeyboardButton(text=f'{name2}')], [KeyboardButton(text=f'{name3}'), KeyboardButton(text=f'{name4}')]]
...
async def sing_up(message: types.Message, state: FSMContext):
    await bot.send_message(message.chat.id, "Введите имя пользователя (только латинский алфавит):")
...
async def set_username(message: types.Message, state: FSMContext):
    if is_included(message.text):
        await bot.send_message(message.chat.id, "Пользователь существует, введите другое имя")
        await state.set_state(RegistrationState.username)
    else:
        await state.update_data(username=message.text)
        await bot.send_message(message.chat.id, "Введите свой email:")
