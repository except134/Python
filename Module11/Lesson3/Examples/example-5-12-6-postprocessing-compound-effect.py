...
from Effects.ColorReduce import ColorReduceEffect
from Effects.Tint import TintEffect
from Effects.Pixelate import PixelateEffect

class Example(Base):
    """
    Demonstrate a compound effect consisting of subsequent
    tint, color reduce, and pixelate effects
    """
    def initialize(self):
        ...
        self.postprocessor.add_effect(TintEffect(tint_color=[0, 1, 0]))
        self.postprocessor.add_effect(ColorReduceEffect(levels=5))
        self.postprocessor.add_effect(PixelateEffect(resolution=[self._screen.get_width(), self._screen.get_height()]))
