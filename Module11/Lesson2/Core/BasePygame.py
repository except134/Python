...
class Base:
    def __init__(self, screen_size=(1920, 1080)):
        ...
        self._screen = pygame.display.set_mode(screen_size, self.display_flags)
        # Set the text that appears in the title bar of the window
        pygame.display.set_caption("Except engine")
        ...
        while self._running:
            for event in pygame.event.get():
                if event.type == pygame.VIDEORESIZE:
                    old_surface_saved = self._screen
                    self._screen = pygame.display.set_mode((event.w, event.h), self.display_flags)
                    self._screen.blit(old_surface_saved, (0,0))
                    del old_surface_saved
                    self.resize(event.w, event.h)
                # process input #
                self._input.update(event)
            if self._input.quit:
                self._running = False
