...
class Example(Base):
    """
    Demonstrate an additive blend effect.
    Note that the top part of the scene contains sky colors,
    and the bottom part contains grass colors.
    Therefore, the top part is brighter, and the bottom part is darker.
    """
    def initialize(self):
        ...
        self.postprocessor.add_effect(
            AdditiveBlendEffect(
                blend_texture=Texture("Data/images/crate.jpg"),
                original_strength=1,
                blend_strength=1
            )
        )
